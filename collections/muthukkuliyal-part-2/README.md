# முத்துக்குளியல் — பாகம் II

Collection/source layer for `முத்துக் குளியல் பாகம் - II` by கலைஞர் மு. கருணாநிதி.

The source is a **36-item multi-speech collection**, not one continuous speech. Controlling PDF: `TVA_BOK_0065802_முத்துக்குளியல்_பாகம்_2.pdf`, SHA-256 `48631b4fc5258df33213dbb742aec70ff6aace4fcd294f377e5750b406f5d9b2`, **425 scans**. Constituent body: PDF **12–424** / printed **11–423**, with stable `PDF = printed + 1`.

## Collection state

- mappings: **36 / 36 complete**
- speech archives started: **2 / 36**
- Tamil T1 complete: **2 / 36**
- Tamil T2/T3 complete: **1 / 36**
- Tamil verified: **1 / 36**
- English E1/E2/E3 complete: **1 / 36**
- fully archived: **1 / 36**

## Constituent 1

`தேசிய இளைஞர் கொண்டாட்டத் தொடக்கவிழா` — PDF 12–18 / printed 11–17 — **CLOSED / FULLY ARCHIVED**.

## Active constituent 2

`முரசொலி அறக்கட்டளை விருது வழங்கு விழா` — `speeches/murasoli-arakkattalai-virudhu-vazhangu-vizha/` — PDF **19–33** / printed **18–32** — 15 pages.

- source gate: **PASS / COMPLETE**;
- date: **13-01-1998** from source closing note;
- venue: not stated in inspected source;
- Tamil T1: **COMPLETE — 15 / 15**;
- T2: **NOT STARTED**;
- T3/English: **BLOCKED**.

## Exact next gate

Constituent 2 Tamil **T2 strict visual audit** over all 15 pages. See `page-map.md` and the speech handover for details.
